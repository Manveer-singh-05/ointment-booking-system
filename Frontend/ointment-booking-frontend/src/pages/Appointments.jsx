...
export default function Appointments() {
  ...
  const cancelBooking = async (bookingId) => {
    const res = await fetch(`http://localhost:4000/bookings/cancel/${bookingId}`, {
      method: "PUT"
    });
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {appointments.map(appt => {
          ...
          const cardClasses = isCancelled
            ? "bg-white/20 backdrop-blur-2xl border border-white/30 shadow-lg opacity-60"
            : "bg-white/30 backdrop-blur-2xl border border-white/40 shadow-xl hover:shadow-[0_0_40px_rgba(0,0,0,0.2)]";
            ...
            >
              <div className="flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-blue-700 font-medium">{pro?.specialization}</p>
                  ...
                  <span
                    className={`inline-block mt-3 px-4 py-1 rounded-full text-sm font-semibold 
                      ${appt.status === "confirmed" ? "bg-green-200 text-green-700"
                      : appt.status === "cancelled" ? "bg-gray-300 text-gray-700"
                ...
                {!isCancelled && (
                  <button
                    ...
                    className="px-6 py-3 bg-red-600 text-white rounded-xl shadow-md 
